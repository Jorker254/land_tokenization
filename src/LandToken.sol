...
contract LandToken is ERC721, Pausable, AccessControl {
    ...
    event LandBoundaryUpdated(uint256 indexed tokenId, string[] polygonCoordinates);
    ...
    function _createGeoLocation(string memory latitude, string memory longitude, string[] memory polygonCoordinates)
        internal
        pure
        returns (GeoLocation memory)
    {
        return GeoLocation({latitude: latitude, longitude: longitude, polygonCoordinates: polygonCoordinates});
    }
    ...
    function mintLandToken(
        ...
        GeoLocation memory geoLocation = _createGeoLocation(latitude, longitude, polygonCoordinates);
    ...
    function updateLandBoundary(uint256 tokenId, string[] memory newPolygonCoordinates) public onlyRole(MINTER_ROLE) {
        require(_ownerOf(tokenId) != address(0), "Token does not exist");
    ...
    function getLandsByLocation(string memory location) public view returns (uint256[] memory) {
    ...
    function verifyLand(uint256 tokenId, bool verified) public onlyRole(MINTER_ROLE) {
        require(_ownerOf(tokenId) != address(0), "Token does not exist");
